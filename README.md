...
В проекте Flink jobs запускаются локально через PyFlink entrypoints, поэтому отдельный Flink Web UI на `localhost:8081` не поднимается. Для просмотра результатов используется Kafka UI на `localhost:8080` и `run-result-consumer`.

Единственное отличие от условия проекта: вместо “тип устройства” у нас используется “производитель устройства”. Это нормально, потому что роль та же самая: manufacturer_id — справочный идентификатор, который потом раскрывается через Postgres. 

Для удобства сверки, что требовалось по заданию / что сделано, есть дополнительный файл - `project_completion_assessment.md`. Это оценка работы со стороны chat-gpt. 
